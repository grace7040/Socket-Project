...
void ServeForever(int threadNum, SOCKET clientSocket, SOCKADDR_IN clientAddress) {
    ...
    while (true) {
        ...
        for (SOCKET& sock : group_queue) {
            if (sock != clientSocket) {
                send(sock, recvData, sizeof(recvData) - 1, 0);
            }
        }
        //종료
    ...
    threadCnt--;
    for (int i = 0; i < group_queue.size(); i++) {
        if (group_queue[i] == clientSocket)
    ...
    while (true) {
        ...
        if (msg == "quit") {
            if (threadCnt == 1) {
                ...
                cout << "> active threads are remaining: " << threadCnt - 1 << " threads" << endl;
            }
        }
    }
